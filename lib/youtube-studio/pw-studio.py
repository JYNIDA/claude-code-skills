...
PROFILE = Path.home() / ".claude/skills/lib/youtube-studio/pw-profile"
...
def cmd_fetch(vid: str, subdir: str):
    ...
    with sync_playwright() as p:
        ...
        if "accounts.google.com" in page.url or "/signin" in page.url:
            print("ERROR: session not logged in. Run: python3 pw-studio.py login", file=sys.stderr)
            context.close()
            sys.exit(2)
