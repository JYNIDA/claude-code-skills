...
`news` 타입으로 선택된 기사 URL에서 이미지를 추출하고 다운로드한다.

**Step 1: 기사 페이지 fetch**

`mcp__fetch__fetch`로 기사 URL을 가져온다.

**Step 2: 이미지 URL 추출**

HTML에서 `<img>` 태그의 `src` 또는 `srcset` 속성을 파싱해 고해상도 이미지 URL을 추출한다.
- `srcset`이 있으면 가장 큰 사이즈 URL 선택
- 썸네일/아이콘(100px 미만 추정) 제외
- 기사 본문 관련 이미지만 선택

**Step 3: curl로 다운로드**

```bash
curl -L -o "[저장경로]/[기사출처]_[설명].jpg" "[이미지 URL]" \
  -H "Referer: [기사 URL]" \
  -H "User-Agent: Mozilla/5.0"
```

파일명 형식: `[출처]_[키워드]_[순번].jpg` (예: `reuters_china_factory_01.jpg`)

**Step 4: 실패 처리**

- 403/401 응답 → 링크만 출처 문서에 기록, 다운로드 불가 표시
- 이미지가 없는 기사 → 기사 링크만 기록
...
- **뉴스 기사 이미지**: fetch → img 태그 파싱 → curl 다운로드. 403 응답 시 링크만 기록. 화질은 웹용(1200px 수준)이므로 방송 사용 시 라이선스 별도 확인.
