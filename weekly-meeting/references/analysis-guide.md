...
### Baseline 계산
- **이번 주(7일) 발행 영상** 중 조회수 상위 N개(`top_n_exclude_for_baseline`, default: 3) 제외
- 나머지 영상의 평균 조회수 = baseline_avg
- 이유: 바이럴 영상이 평균을 왜곡하는 것을 방지
- 참고: 30일 데이터는 캐리오버 영상 추적용으로만 사용
...
적용 대상: Weekly Views (7d), Subscribers, Shorts Avg, Long-form Views
...
- **강한 주**: "Strong week — Weekly Views {X}K, {long}x long-form + {short}x shorts, top performer {title} at {views} in {days} days"
- **Mega-hit 발생**: "Breakout week — {count} mega-hit(s) led by \"{title}\" ({views} views)"
- **지난 주 대비 하락 (컨텍스트 필요)**: "Transition week — Weekly Views {X}K vs {Y}K prev wk. Context: {reason for prev week's higher/lower numbers}"
- **하락세**: "Challenging week — Weekly Views down {X}% vs prev wk, {count} underperformers flagged"
- **안정적**: "Steady week — Weekly Views {X}K, consistent with prev wk ({Y}K), no significant outliers"
...
| 영상 0개 (7일) | "No videos published this week" 경고 |
