---
name: ttm-pipeline
description: The Thinking Mode 시리즈 콘텐츠 파이프라인. 트렌드 스캔 → 게스트 검증 → 의사결정 → 아웃리치 → 심층 리서치 → 질문지. "ttm pipeline", "thinking mode pipeline", "게스트 발굴", "파이프라인", "guest validation", "게스트 검증", "섭외 파이프라인", "ttm research", "게스트 리서치" 요청에 사용.
user_invocable: true
---
...
```
/ttm-pipeline                          → 스테이지 선택 메뉴 표시 (아래 참조)
/ttm-pipeline trend                    → Stage 1: Trend Radar
/ttm-pipeline validate <name>          → Stage 2: Guest Validation
/ttm-pipeline validate-batch           → Stage 2-Batch: 여러 게스트 동시 검증
/ttm-pipeline decide                   → Stage 3: Decision Gate
/ttm-pipeline outreach <name>          → Stage 4: Outreach Email
/ttm-pipeline research <name>          → Stage 5: Deep Research + 9-Section Brief
/ttm-pipeline questions <name>         → Stage 6: Interview Questions
```

**인자 없이 호출 시 — 스테이지 선택 메뉴를 즉시 출력한다 (AskUserQuestion 사용 금지):**

```
━━━ TTM Pipeline ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  1. Trend Radar          트렌드 스캔 + 핫이슈 발굴
  2. Guest Validation     게스트 검증 (5-Dimension + Yama)
  3. Decision Gate        Go / Maybe / Pass 결정
  4. Outreach Email       섭외 이메일 초안
  5. Deep Research        9-Section 빌링구얼 리서치 브리프
  6. Interview Questions  질문지 작성
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
어느 단계부터 시작할까요? (번호 또는 게스트 이름 입력)
```

게스트 이름이 포함된 입력이면 Stage 2부터 자동 시작. 번호 선택 후 필요한 추가 정보(게스트 이름 등)는 그때 묻는다.

---

## 시리즈 컨텍스트

**Read before starting any stage:** `~/.claude/skills/references/ttm_series_context.md`
This file has the 6-theme structure, Beyond Question framework, confirmed episodes, and pipeline. Single source of truth — do not reconstruct from memory.
...
| **벤치마크** | ep.1 Po-Shen Loh: 1M+ views |
| **타겟** | AI 시대에 대체 불가능한 해법을 찾는 고지능 실용주의 성취자 |
| **듀얼 레이어** | 표면=실용적 AI 리터러시 / 깊은 층=정체성·사고방식 전환 |
| **6 Themes** | After Coding / Redesigning Learning / When AI Dreams / The Future of Work / A New Map of Power / The Architects |
| **Priority gaps** | Theme 3 (When AI Dreams), Theme 6 (The Architects) — no confirmed episodes |

4 Core Personas + Theme notes: see `ttm_series_context.md`
...
## Beyond Question — Theme Mapping

Before validating a guest, identify their theme and Beyond Question from `ttm_series_context.md`.

**Why**: The Beyond Question anchors the episode before the guest is chosen. Validation scores a guest's fit against the question — not just their general prestige.

**Quick check**: "Can only this guest answer this question this way?" If no → sharpen the question or find a better fit.

**Theme gap**: Themes 3 and 6 have no confirmed episodes — prioritize candidates for these first.
...
**반드시 `~/.claude/skills/references/eo_interview_research_workflow.md`를 먼저 읽고** Phase 1 + Phase 2를 실행한다.
...
### 출력 형식 — 9-Section Bilingual Research Brief

**BILINGUAL FORMAT — MANDATORY, NO EXCEPTIONS:**
모든 섹션의 모든 문장/데이터/항목을 영어 먼저, 한국어 번역 바로 아래에 병기한다.

> [English content]
>
> 🇰🇷 [한국어 번역]

한국어가 빠진 섹션은 불완전한 것으로 간주. 테이블, 불릿, 인용문도 예외 없음.

---

**§1 — Disambiguation**
동명이인이 있을 경우 대상을 명시. 판별 기준 + 링크 포함.

**§2 — Person Deep Dive**
- 2a. Current Role — 직함, 기관, 위치, 재직 기간
- 2b. Career Timeline — 표: Period | Role | Organization
- 2c. Research/Work Footprint — 주요 논문(인용 수), 제품, 강연, 특허
  - 학자: h-index, 총 인용 수, 대표 논문
  - 실무자: 출시 제품, 측정 가능한 임팩트
- 2d. Public Stance & Themes — 핵심 지적 주장 2-3개. 남들이 동의하지 않는 포지션.
- 2e. Network — 주요 협력자, 멘토, 펀더, 비판자

**§3 — Company / Project Deep Dive**
스타트업이면: 사업 내용, 창업 스토리, 비즈니스 모델, 펀딩, 트랙션.
학자면: 랩 + 연구 아젠다.

**§4 — What's New (Last 90 Days)**
논문, 강연, 인터뷰, 팟캐스트, 주요 보도, 론치, 논쟁. 날짜 명시.

**§5 — Open Questions & Diligence Flags**
번호 목록. 미확인 펀딩, 불명확한 팀 규모, 미해결 논쟁, 검증 안 된 주장 플래그.

**§6 — TTM Fit Analysis ⭐ (가장 중요)**
- 6a. Theme — 6개 TTM 테마 중 해당 테마. 구체적 작업 2-3개 인용. 멀티테마면 주 테마 선택. `ttm_series_context.md`의 확정 게스트와 차별점 확인.
- 6b. Core Question — *"[이 게스트의 작업]이 사실이라면, 창업가/빌더/사상가에게 무엇을 의미하는가?"*
- 6c. Arc Weight — 에피소드 아크 ①-⑥ 중 이 게스트에게 가장 비중을 둬야 할 2-3개 파트. 이유 포함.
- 6d. Series Angle — Unlearning / Access & Equity / Frontier Builder / Human Resistance / Governance Gap 중 선택 + 연결 설명.

**§7 — Framing + Yama**
프로듀서를 위한 2-3문장: 이 사람이 누구인지, 왜 지금인지, 에피소드의 진짜 주제가 무엇인지.

**Yama:**
> **"[이 게스트 작업에 특유한 긴장 또는 역설]?"**

**§8 — YouTube Titles & Thumbnail Phrases**
제목 8개. TTM 보이스: 선정주의 없음, 둠 없음, Practical Optimism.
빌더/창업가가 한 시간을 비울 만큼 끌려야 함.

한국어 썸네일 문구 5개 (2-8자, 임팩트 있게, 얼굴 샷 오버레이 텍스트로 작동).

**§9 — Outreach Email Draft**
Stage 4 아웃리치 이메일 포맷(`references/outreach-email-template.md`) 참조하여 작성.
- 고정 오프닝 문단 (템플릿 그대로 사용)
- Hook 단락: 가장 최근 작업에 구체적으로 앵커. 복습이 아닌 새로운 대화 시그널.
- Ask + Closing
- 제목 3개 옵션 (기본: `Interview Invitation: EO x [Guest Name]`)
- 7일 팔로업 이메일 초안

---

**저장 위치 (두 곳 모두):**
1. **Notion:** TTM Research Brief 페이지 (ID: `32574768-ec37-80f3-b782-f49ff8c989bd`) 맨 아래에 추가. `---` 구분선 후 append.
2. **로컬 파일:** `~/Desktop/Cowork/Research workflow/validation_[GuestName]_[YYYY-MM-DD].md`
...
3. `~/.claude/skills/references/eo_interview_research_workflow.md` Phase 3 절차 따르기
...
| `~/.claude/skills/references/eo_interview_research_workflow.md` | 심층 리서치 방법론 |
