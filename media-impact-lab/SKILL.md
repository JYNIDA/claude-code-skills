...
Persistent Chromium profile lives at `~/.claude/skills/lib/youtube-studio/pw-profile/`. Cookies persist across runs, typically months. When they eventually expire, the fetch command prints `ERROR: session not logged in` — re-run `login` mode and the scrape keeps working.

**Deprecated** (do not use): `fetch-reach.sh`, `check-auth.sh`, `import-cookies.sh` in the same directory. They relied on gstack browse which is fragile. Keep them for now so old scripts don't break, but all new runs go through `pw-studio.py`.
